...
void CLCD_voidInit (void)
{
	...
	/* [00.] Must wait more than 30 ms before any action (until VDD rises to 4.5 v) */
	_delay_ms(50);

	/* All Pins as OutPut pins */
	DIO_enumSetPortDirection ( CLCD_DATA_PORT    , DIO_PORT_OUTPUT           ); /* DATA PORT */
	DIO_enumSetPinDirection  ( CLCD_CONTROL_PORT , CLCD_RS , DIO_PIN_OUTPUT  ); /* CONTROL PIN: RS */
	DIO_enumSetPinDirection  ( CLCD_CONTROL_PORT , CLCD_RW , DIO_PIN_OUTPUT  ); /* CONTROL PIN: RW */
	DIO_enumSetPinDirection  ( CLCD_CONTROL_PORT , CLCD_EN , DIO_PIN_OUTPUT  ); /* CONTROL PIN: EN */

	/* Return cursor to the first position on the first line    */
	CLCD_voidSendCommand( lcd_Home );
	_delay_ms(1);						/* wait more than 39 Ms */

	/* [01.] FUNCTION SET Command : 2 lines , 5*8 font size		*/
	CLCD_voidSendCommand( EIGHT_BITS ); /* 8 Bit Mode ==> 0x38  */
	_delay_ms(1);						/* wait more than 39 Ms */

	/* [02.] DISPLAY & Cursor (ON / OFF) Control */
	CLCD_voidSendCommand( lcd_DisplayOn_CursorOff );
	_delay_ms(1);						/* wait more than 39 Ms */

	/* [03.] DISPLAY CLEAR */
	CLCD_voidClearScreen(); /* Implemented later in the code + the 1.53ms Delay */
...
{
	u64 LOC_u64Reversed = 1 ; /* Avoid ignorance of left side zeros (1 0001) */
	...
	else
	{
		...
		while( LOC_u64Reversed != 1 ) /* Make sure the 1 don't corrupt data*/
		{
...
void CLCD_voidSendExtraChar( u8 Copy_u8Row , u8 Copy_u8Col )
{
	...
	/* 2- Draw All Characters in CGRAM */
...
/*===============================================================================================================================*/
/*
* Brief : This Private Function send a pulse (falling edge ) to Enable Pin
* Parameters : nothing
* return : nothing
*
